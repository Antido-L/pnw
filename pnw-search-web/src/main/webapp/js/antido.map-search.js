...
function MapUtil() {
	...
	this.showParkInMap = function(x,y,name,i,desc,parkId) {
		if(x == null || x == "" || x == 0.0) {
			return;
		}
		...
		marker.addEventListener("click", function() {
			window.open ('http://localhost:8086/?parkId='+parkId);
		});
	    //numLabel.setOffset(new BMap.Size(5,2));
...
function focusMap(index) {
	switch(index) {
		case "1": {
			map.panTo(map.marker_1.point,12);
			break;
		}
		case "2": {
			map.panTo(map.marker_2.point,12);
			break;
		}
		case "3": {
			map.panTo(map.marker_3.point,12);
			break;
		}
		case "4": {
			map.panTo(map.marker_4.point,12);
			break;
		}
		case "5": {
			map.panTo(map.marker_5.point,12);
			break;
		}
		case "6": {
			map.panTo(map.marker_6.point,12);
			break;
		}
		case "7": {
			map.panTo(map.marker_7.point,12);
			break;
		}
		case "8": {
			map.panTo(map.marker_8.point,12);
			break;
		}
		default : break;
	}
