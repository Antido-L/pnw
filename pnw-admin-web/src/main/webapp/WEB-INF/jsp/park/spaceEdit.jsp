...
<body>
		            	...
		            	<div class="panel panel-default">
				            ...
				            <div class="panel-body">
								...
								<div class="col-sm-12" style="padding-left: 0;padding-bottom: 10px">
					              ...
					              <button style="margin: 15px auto;" class="col-sm-12 btn btn-primary btn-sm" onclick="location.reload()">刷新状态</button>
					              
				                  <button style="margin: 0 auto;" class="col-sm-12 btn btn-danger btn-sm">重置所有</button>
	...
	<script>
		//在停车场生成页面画出模型图
			...
			//可规定模型图的长宽比
			//可设定模型图的尺寸
			//车位槽在哪里生成在停车场生成页面就进行绑定
				//加载车位槽时自动生成未绑定的空车位
				//生成一张临时的车位槽分布表
				//通过控制器获取节点列表 在停车场车位编辑页面对车位和节点的绑定工作
		/*页面加载完毕*/		
		$().ready(function() {
			//ajaxForm 参数
			var options = {
		...
		function updateMapItem(data) {
			...
			if (data.node != null && data.node.id != "") { //有节点
				if(data.node.is_online == '1') { //节点已经连接
					...
					/*$("#item-"+data.x_axis+"-"+data.y_axis).removeClass();
					$("#item-"+data.x_axis+"-"+data.y_axis).addClass("mod-item-success"); */
				} else { //节点未连接
					$("#item-"+data.x_axis+"-"+data.y_axis+"> div").removeClass();
