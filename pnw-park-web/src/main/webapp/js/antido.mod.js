...
var ModUtils = {
	...
	modMaker:function(width,height,row,col,ratio) {
		...
		for(var x = 0 ; x < row ; x++) {
			...
			for(var y = 0 ; y < col ; y++) {
				var $item = $("<div id='item-"+x+"-"+y+"' class='mod-item' onclick='ModUtils.clickItem("+x+","+y+")' onMouseover='ModUtils.mouOver("+x+","+y+")' onMouseout='ModUtils.mouOut("+x+","+y+")'></div>");
				//设置初始宽高
				$item.width(width);
				$item.height(height);
				$rowDiv.append($item);
		...
		$.ajax({
			url:this.param.contextPath + "/allSpaceByParkId",
  			...
  			success:function(data) { //包含当前停车场内车位列表
  				...
  				var total = 0;
  				for(var i = 0 ; i < data.length ; i++) {
  					//$("#item-"+data[i].x_axis+"-"+data[i].y_axis).find("div[class='mod-item-empty']").attr("onclick","ModUtils.clickItem('"+data[i].x_axis+"','"+data[i].y_axis+"')");
	...
	clickItem:function (x,y) {
		...
		if(UserUtils.user != null && UserUtils.user.space != null) {
			if(UserUtils.user.space.idStr == spaceId) {
				ModUtils.showUsingState(UserUtils.user.space,UserUtils.user);
				return;
			}
		}
  		...
  		$.ajax({
  			...
  			success:function(data){
  				...
  				if(data.space.using_state == 0) { //该车位空闲
  					...
  					var infoContent = "";
  					var isMystery = true;
  					if(data.user.car.car_desc != null) {
  						infoContent += "<h4>"+ data.user.car.car_desc +"</h4>"
  					} else {
  						infoContent += "<p>神秘车辆<p>"
  					}
  					
  					if(data.user.show_name) {
  						infoContent += "<p>车主："+ data.user.nick_name +"<p>"
  						isMystery = false;
  					} 
  					if(data.user.show_license) {
  						infoContent += "<p>车牌号："+ data.user.car.license +"<p>"
  						isMystery = false;
  					}
  					if(data.user.show_phone) {
  						infoContent += "<p>预留电话："+ data.user.phone +"<p>"
  						isMystery = false;
  					}
  					if(isMystery) infoContent += "<p>该用户很低调<p>"
  						
  					if(data.leavingTimeStr != null) {
  						infoContent += "<p>预计离开："+ data.leavingTimeStr +"<p>"
  					}
  					
  					$("#usingCheck-title-spaceCode").html(data.space.code);
  					//弹出操作框
  					$("#usingCheck-content").html(infoContent);
	...
	showUsingState:function(space,user) {
		//当用户未确认时提醒确认
		if(space.node.parent_id == null) {
			$("#model-user-parkInfo-hidden-controllerId").val(space.park.node.id);
		} else 
